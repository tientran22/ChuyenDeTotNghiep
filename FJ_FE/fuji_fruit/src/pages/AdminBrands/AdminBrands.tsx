...
import ModalViewBrand from './components/ModalViewBrand'
...
export default function AdminBrands() {
  ...
  const openViewModal = async (brandId: string) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/admin/brands/${brandId}`) // Thay đổi URL của API theo đường dẫn và tham số cụ thể của bạn
      const brandData = response.data.data // Dữ liệu chi tiết sản phẩm từ API
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`http://127.0.0.1:8000/api/admin/brands/${selectedItemId}`)
    ...
    <div>
      ...
      <section className='bg-gray-50 dark:bg-gray-900 p-3 sm:p-5 antialiased'>
        <div className='flex items-center justify-end mx-12 my-4'>
          ...
          <FormCreateBrand isModalOpen={isModalOpen} closeModal={closeModal} />
        </div>
          ...
          <div className='bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden'>
            <div className='overflow-x-auto'>
              <table className='w-full text-sm text-left text-gray-500 dark:text-gray-400'>
                ...
                <tbody>
                  {adminBrandData.data.data.map((brand) => (
                    <tr className='border-b dark:border-gray-700' key={brand.id}>
                      <td className='px-4 py-3 text-center'>{brand.name}</td>

                      <td className='px-4 py-3 flex items-center justify-center text-center'>
                        <button
                          onClick={() => openAction(brand.id)}
                        ...
                        >
                          ...
                          {selectedItemId === brand.id && isActionOpen && (
                            <div
                              ref={actionRef}
                            ...
                            >
                              <ul className='py-1 text-sm' aria-labelledby='apple-imac-27-dropdown-button'>
                                ...
                                <li>
                                  <button
                                    onClick={() => openViewModal(brand.id)}
                                    type='button'
      ...
      <ModalViewBrand
        ...
        brandDetail={brandDetail as Brand}
        ...
        >
          <div className='relative p-4 w-full max-w-md max-h-full'>
            ...
            <div className='relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5'>
              ...
              <p className='mb-4 text-gray-500 dark:text-gray-300'>Bạn có muốn xóa thương hiệu này?</p>
