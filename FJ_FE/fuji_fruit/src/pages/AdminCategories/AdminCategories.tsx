...
export default function AdminBrands() {
  ...
  const { data: adminCategoryData, refetch } = useQuery({
    queryKey: ['adminCategory', queryConfig],
    queryFn: () => AdminCategoryApi.getCategories(queryConfig as ProductListConfig)
  })
  ...
  const openAction = (categoryId: string) => {
  ...
  const openViewModal = async (categoryId: string) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/admin/categories/${categoryId}`) // Thay đổi URL của API theo đường dẫn và tham số cụ thể của bạn
      const categoryData = response.data.data // Dữ liệu chi tiết sản phẩm từ API
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`http://127.0.0.1:8000/api/admin/categories/${selectedItemId}`)
  ...
  if (!adminCategoryData) return null
    ...
    <div>
      ...
      <section className='bg-gray-50 dark:bg-gray-900 p-3 sm:p-5 antialiased'>
        <div className='flex items-center justify-end mx-12 my-4'>
          ...
          <FormCreateCategory isModalOpen={isModalOpen} closeModal={closeModal} />
        </div>
          ...
          <div className='bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden'>
            <div className='overflow-x-auto'>
              <table className='w-full text-sm text-left text-gray-500 dark:text-gray-400'>
                ...
                <tbody>
                  {adminCategoryData.data.data.map((category) => (
                    <tr className='border-b dark:border-gray-700' key={category.id}>
                      ...
                      <td className='px-4 py-3 flex items-center justify-center text-center'>
                        <button
                          onClick={() => openAction(category.id)}
                        ...
                        >
                          ...
                          {selectedItemId === category.id && isActionOpen && (
                            <div
                              ref={actionRef}
      ...
      <ModalViewCategory
        ...
        categoryDetail={categoryDetail as Category}
