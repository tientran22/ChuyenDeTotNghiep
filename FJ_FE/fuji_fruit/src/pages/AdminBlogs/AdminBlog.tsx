...
import FormCreateBlog from './components/FormCreateBlog/FormCreateBlog'
import FormEditBlog from './components/FormEditBlog'
import ModalViewBlog from './components/ModalViewBlog'
...
export default function AdminBlogs() {
  ...
  const { data: adminBlogData, refetch } = useQuery({
    queryKey: ['adminBlog', queryConfig],
    queryFn: () => AdminBlogApi.getBlogs(queryConfig as ProductListConfig)
  })
  ...
  const openViewModal = async (blogId: string) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/admin/blogs/${blogId}`) // Thay đổi URL của API theo đường dẫn và tham số cụ thể của bạn
      const blogData = response.data.data // Dữ liệu chi tiết sản phẩm từ API
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`http://127.0.0.1:8000/api/admin/blogs/${selectedItemId}`)
    ...
    <div>
      ...
      <section className='bg-gray-50 dark:bg-gray-900 p-3 sm:p-5 antialiased'>
        <div className='flex items-center justify-end mx-12 my-4'>
          ...
          <FormCreateBlog isModalOpen={isModalOpen} closeModal={closeModal} />
        </div>
          ...
          <div className='bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden'>
            <div className='overflow-x-auto'>
              <table className='w-full text-sm text-left text-gray-500 dark:text-gray-400'>
                ...
                <tbody>
                  {adminBlogData.data.data.map((blog) => (
                    <tr className='border-b dark:border-gray-700' key={blog.id}>
                      <td className='px-4 py-3 text-center'>{blog.title}</td>
                      <td className='px-4 py-3 text-center  max-w-[12rem] truncate'>{blog.content}</td>

                      <td className='px-4 py-3 text-center  max-w-[12rem] truncate'>{blog.description}</td>
                      ...
                      <td className='px-4 py-3 flex items-center justify-center text-center'>
                        ...
                        >
                          ...
                          {selectedItemId === blog.id && isActionOpen && (
                            <div
                              ref={actionRef}
        ...
        >
          <div className='relative p-4 w-full max-w-md max-h-full'>
            ...
            <div className='relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5'>
              ...
              <p className='mb-4 text-gray-500 dark:text-gray-300'>Bạn có muốn xóa tin tức này?</p>
